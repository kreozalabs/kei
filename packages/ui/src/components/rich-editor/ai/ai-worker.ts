...
self.onmessage = async (event: MessageEvent) => {
  ...
  if (type === "extract-entities") {
    ...
    try {
      ...
      const filtered = entityTypes
        ? entities.filter((e: { entity: string }) => entityTypes.includes(e.entity))
        : entities;
  ...
  try {
    ...
    if (stoppedEarly) {
      ...
      const generatedMessages = (result as { generated_text: unknown }).generated_text;
    ...
    if (stoppedEarly) {
      ...
      const stopIndex = accumulated.indexOf(stopAt);
      const fullText = stopIndex >= 0 ? accumulated.slice(0, stopIndex).trim() : accumulated.trim();
